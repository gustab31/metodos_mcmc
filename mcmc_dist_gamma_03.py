...
# =====================================================
# Dados de entrada
input_file_path = "C:\\Users\\gusta\\Dropbox\\PC\\Documents\\mestrado\\etapa_de_programacao\\programacao_gamma_mcmc\\time_dnit.csv"
...
# Vamos aplicar uma distribuicao gamma aos dados  
modelo=lambda t:np.random.gamma(mean,sd,t)
...
# =====================================================
# Modelo de transicao 
modelo_trans = lambda x: [x[0],np.random.gamma(x[1],0.5,(1,))[0]]
...
def prob_gamma_log(x,data):
    #x[0]=mu, x[1]=sigma 
    #data = a mesma observada
    return np.sum(np.log(scipy.stats.gamma(x[0],x[1]).pdf(data)))
...
aceitado, rejeitado = metropolis_hastings(prob_gamma_log,prior,modelo_trans,[mu_obs,0.1], 10000,observado,aceitacao)# 50000,observado,aceitacao)
aceitado[-10:,1]
...
ax.set_ylabel("Frequencia (gamma)")
ax.set_xlabel("$\sigma$")
...
with open('dist_gamma.txt', 'w') as output:
