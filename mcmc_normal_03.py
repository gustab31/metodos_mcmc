...
# =====================================================
# Dados de entrada
input_file_path = "C:\\Users\\gusta\\Dropbox\\PC\\Documents\\mestrado\\etapa_de_programacao\\programacao_normal_mcmc\\time_dnit.csv"
...
# Vamos aplicar uma distribuicao normal aos dados  
modelo=lambda t:np.random.normal(mean,sd,t)
...
# =====================================================
# Modelo de transicao 
modelo_trans = lambda x: [x[0],np.random.normal(x[1],0.5,(1,))[0]]
...
def manual_log_prob_normal(x,data):
    #x[0]=mu, x[1]=sigma 
    #data = a mesma observada
    return np.sum(-np.log(x[1] * np.sqrt(2* np.pi) )-((data-x[0])**2) / (2*x[1]**2))   # testar com math.log 

# Mesma que a usada anteriormente, mas testaremos com a biblioteca scipy.
def prob_normal_log(x,data):
    #x[0]=mu, x[1]=sigma 
    #data = a mesma observada
...
aceitado, rejeitado = metropolis_hastings(manual_log_prob_normal,prior,modelo_trans,[mu_obs,0.1], 10000,observado,aceitacao)# 50000,observado,aceitacao)
aceitado[-10:,1]
...
ax.set_ylabel("Frequencia (normal)")
ax.set_xlabel("$\sigma$")
...
with open('dist_normal.txt', 'w') as output:
